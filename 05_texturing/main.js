...
function createSceneGraph(gl, resources) {
  ...
  {
    //initialize light
    let light = new LightSGNode(); //use framework implementation of light node
...

  {
    ...
    root.append(new TransformationSGNode(glm.transform({ translate: [-4,1,0], rotateX: 90, scale: 1}), [
      floor
    ]));
  }
...
function initTextures(resources)
{
  ...
  gl.texImage2D(gl.TEXTURE_2D, //texture unit target == texture type
    ...
    resources.texture_normal); //actual image data
